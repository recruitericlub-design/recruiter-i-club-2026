...
export default function Navbar() {
  ...
  const navLinks = isRu
    ? [
        ...
        { name: 'Гарантии', href: '/ru#guarantees' },
        ...
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-3">
            <span className="inline-flex items-center gap-1.5 font-semibold text-amber-400">
              ...
              {isRu ? 'Официальный партнер работодателей Украины' : 'Офіційний партнер роботодавців України'}
            </span>
            ...
            <span className="hidden sm:inline-flex items-center gap-1.5 text-slate-200">
              ...
              {isRu 
                ? 'Лицензия Минсоцполитики №1428 · 100% защита от мобилизации (ст. 23 ЗУ)'
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden lg:flex items-center gap-3">
              <div className="flex flex-col text-right pr-2">
                <span className="text-[11px] text-slate-500 font-medium">{isRu ? 'Стоимость найма' : 'Вартість найму'}</span>
                <span className="text-xs font-bold text-slate-900">{isRu ? 'от $500 / рабочий' : 'від $500 / працівник'}</span>
              </div>
          ...
          <div className="xl:hidden bg-white border-b border-slate-200 px-4 pt-2 pb-6 space-y-1 shadow-xl">
            ...
            <div className="pt-3 border-t border-slate-100 flex flex-col gap-2">
              <div className="flex items-center justify-between px-2 py-1">
                <span className="text-xs font-bold text-slate-600">Мова / Язык:</span>
                <div className="inline-flex items-center rounded-lg border border-slate-200 bg-slate-100 p-0.5 text-xs font-bold">
                  <Link
                    href="/"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className={`px-3 py-1 rounded-md transition ${!isRu ? 'bg-white shadow-xs text-blue-700 font-black' : 'text-slate-500'}`}
                  >
                    UA
                  </Link>
                  <Link
              ...
              >
                Отримати анкети кандидатів (без передплати)
              </button>
            </div>
          </div>
        )}
